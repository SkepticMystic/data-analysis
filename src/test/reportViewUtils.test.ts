import {buildPrintableCorrs, getReportCorrs, ALL_FIELDS} from "../reportViewUtils";
import { shortCorrelations } from "./testData";

test('buildReportCorrs properly filters out correlations below a min and max', () => {
    const results = getReportCorrs(buildPrintableCorrs(shortCorrelations), ALL_FIELDS, 0.8, 0.44);
...
test('buildReportCorrs properly flattens list of Correlations', () => {
    const results = getReportCorrs(buildPrintableCorrs(shortCorrelations), ALL_FIELDS, 0.5, -0.1);
...
test('buildReportCorrs filters out non-selected fields', () => {
    const results = getReportCorrs(buildPrintableCorrs(shortCorrelations), "numberField1", -0.3, -0.8);
    expect(results.length).toEqual(2);
