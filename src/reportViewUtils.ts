...
export const buildReportCorrs = (corrs: Correlations, selectedField: string, max: number, min: number): PrintableCorrelation[] => {
...
export const top3PositiveCorrs = (correlations: PrintableCorrelation[], sorted: boolean = true): PrintableCorrelation[] => {
    if (!sorted) {
...
export const top3NegativeCorrs = (correlations: PrintableCorrelation[], sorted: boolean = true): PrintableCorrelation[] => {
    if (!sorted) {
        correlations.sort((a, b) => b.info.corr - a.info.corr);
    }
